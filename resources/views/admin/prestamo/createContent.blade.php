                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="date" class="form-control @error('fecha_fin') is-invalid @enderror"
                                ...
                                value="{{ old('fecha_fin') }}" placeholder="Seleccione la fecha de finalización">
                            @error('fecha_fin')
                                <span class="invalid-feedback d-block" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="date" class="form-control @error('fecha_inicio') is-invalid @enderror"
                                ...
                                value="{{ old('fecha_inicio') }}" placeholder="Seleccione la fecha de inicio">
                            @error('fecha_inicio')
                                <span class="invalid-feedback d-block" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" name="descripcion"
                            rows="3" placeholder="Ingrese una descripción del préstamo" maxlength="100">{{ old('descripcion') }}</textarea>
                        <small class="text-muted float-right"><span
                                id="contador-caracteres">{{ strlen(old('descripcion', '')) }}</span>/100
                            caracteres</small>
                        @error('descripcion')
                            <span class="invalid-feedback d-block" role="alert">
                                <strong>{{ $message }}</strong>
                        ...
                        <div class="row libro-item mb-3">
                            <div class="col-md-5">
                                <div class="form-group">
                                    <label>Libro</label>
                                    <select
                                        class="form-control select-libro @error('libros.0.libro_id') is-invalid @enderror"
                                        name="libros[0][libro_id]" required>
                                        <option value=""
                                            {{ old('libros.0.libro_id') ? '' : 'selected disabled' }}>Seleccione un
                                            libro</option>
                                        @foreach ($libros as $libro)
                                            <option value="{{ $libro->id }}"
                                                {{ old('libros.0.libro_id') == $libro->id ? 'selected' : '' }}>
                                                {{ $libro->titulo }} - {{ $libro->autor }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number"
                                        class="form-control cantidad-libro @error('libros.0.cantidad') is-invalid @enderror"
                                        name="libros[0][cantidad]" min="1"
                                        value="{{ old('libros.0.cantidad', 1) }}" required>
